...
from MozaikLite.framework.decorators import timeit
...
@timeit
def stRF_kernel_2d(duration=200.0, dt=1000.0/120.0, size=10.0, scale_factor=10.0, p={}):
    """
    scale_factor = pixel/degree
    """
    degree_per_pixel = 1./float(scale_factor)
    #p = RF_parameters()
    x = numpy.arange(-size/2.+degree_per_pixel/2.,size/2.+degree_per_pixel/2.,degree_per_pixel)
...
def G(t, K1, K2, c1, c2, t1, t2, n1, n2):
    p1 = K1*(((c1*(t-t1))**n1)*numpy.exp((-c1*(t-t1))))/((n1**n1)*numpy.exp(-n1))
    p2 = K2*(((c2*(t-t2))**n2)*numpy.exp((-c2*(t-t2))))/((n2**n2)*numpy.exp(-n2))
    p3 = p1-p2
    ### norm to max == 1.
    ##p3 /= p3.max()
    return p3
...
def F_2d(x, y, A, sigma):
    return A*numpy.exp(-(x**2+y**2)/(2*sigma**2))
