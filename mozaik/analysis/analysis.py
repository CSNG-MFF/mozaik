...
class Analysis(ParametrizedObject):
    ...
    the :meth:`perform_analysis` function which should contain the analysis code.
    This function should retrieve data from the :class:`~mozaik.core.DataStoreView`
    that is supplied in the ``datastore`` parameter. Further, it should include ``tags``
    as the tags for all :class:`~mozaik.analysis.AnalysisDataStructure` objects that it creates.
    (See the description of the :attr:`~mozaik.analysis.AnalysisDataStructure.tags` attribute).
