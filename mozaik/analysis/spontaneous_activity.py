    ...
    def perform_analysis(self):
        ...
        for sheet in self.datastore.sheets():
            ...
            assert len(dsv_cv.get_analysis_result()) == 1, (
                "Error: SpontaneousActivityLength accepts only datastore that holds one  SingleValue analysis data structure with value_name: Mean(CV of ISI squared). It contains: %d"
                % len(dsv_cv.get_analysis_result())
            )
            assert len(dsv_corr.get_analysis_result()) == 1, (
                "Error: SpontaneousActivityLength accepts only datastore that holds one  SingleValue analysis data structure with value_name: 'Mean(Correlation coefficient(psth (bin=5.0))).It contains: %d"
                % len(dsv_cv.get_analysis_result())
            )
            assert len(dsv_psth.get_analysis_result()) == 1, (
                "Error: SpontaneousActivityLength accepts only datastore that holds one  AnalogSignal analysis data structure with value_name: 'Mean(psth (bin=5.0)).It contains: %d"
                % len(dsv_cv.get_analysis_result())
            )
