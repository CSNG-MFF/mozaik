...
def single_value_visualization(
        ...
        for d in datastores:
            value_names.update(
                set(
                    [
                        ads.value_name
                        for ads in param_filter_query(
                            d[1], identifier="SingleValue"
                        ).get_analysis_result()
                    ]
                )
            )
    ...
    for (param_values, datastore) in datastores:
        for v in value_names:
            if (
                len(
                    param_filter_query(
                        datastore, identifier="SingleValue", value_name=v
                    ).get_analysis_result()
                )
                > 1
            ):
    ...
    for i, value_name in enumerate(value_names):
        ...
        if len(parameters) == 1:
            ...
            z = []
            for (param_values, datastore) in datastores:
                adss = param_filter_query(
                    datastore, identifier="SingleValue", value_name=value_name
                ).get_analysis_result()
                if len(adss) > 0:
...
def fixed_point_visualization(
    simulation_name, rate_name, master_results_dir, query, filename=None
):
    """
    Visualizes all single values (or those whose names match ones in `value_names` argument)
    present in the datastores of parameter search over a fixed set of parameters. 
    ...
    value_names = set(
        [
            ads.value_name
            for ads in param_filter_query(
                datastores[0][1], identifier="SingleValue"
            ).get_analysis_result()
        ]
    )
    ...
    for (param_values, datastore) in datastores:
        ...
        for v in value_names:
            assert (
                len(
                    param_filter_query(
                        dsv, identifier="SingleValue", value_name=v
                    ).get_analysis_result()
                )
                == 1
            ), (
                ...
                % (
                    len(
                        param_filter_query(
                            datastore, identifier="SingleValue"
                        ).get_analysis_result()
                    ),
                    str(
                        [
...
def multi_curve_visualzition(
    ...
    for (param_values, datastore) in datastores:
        ...
        assert (
            len(
                param_filter_query(
                    dsv, identifier="SingleValue", value_name=value_name
                ).get_analysis_result()
            )
            == 1
        ), (
    ...
    for (param_values, datastore) in datastores:
        ...
        t[1].append(
            float(
                param_filter_query(dsv, identifier="SingleValue", value_name=value_name)
                .get_analysis_result()[0]
                .value
            )
        )
        d[tuple(key)] = t
