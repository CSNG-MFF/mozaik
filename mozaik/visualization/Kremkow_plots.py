...
class Figure2Gratings(Plotting):
      ...
      def subplot(self,subplotspec,params):
          ...
          ConductanceSignalListPlot(queries.TagBasedQuery(ParameterSet({'tags' : ['GSTA'] })).query(self.datastore),ParameterSet({'sheet_name' : self.parameters.sheet_name,'normalize_individually':True})).subplot(gs[7:10,15:],params.copy())  
...
class Figure2NaturalImagesWithEyeMovement(Plotting):
      ...
      def subplot(self,subplotspec,params):
          ...
          dsv = queries.select_stimuli_type_query(self.datastore,'NaturalImageWithEyeMovement',{})
