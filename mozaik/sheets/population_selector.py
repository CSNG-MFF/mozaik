...
class RCGrid(PopulationSelector):
    ...
    def generate_idd_list_of_neurons(self):
        ...
        for x in (
            ...
            + numpy.arange(0, self.parameters.size, self.parameters.spacing)
            ...
            for y in (
                ...
                picked.append(
                    z[
                        numpy.argmin(
                            numpy.power(self.sheet.pop.positions[0] - xx, 2)
                            + numpy.power(self.sheet.pop.positions[1] - yy, 2)
                        )
                    ]
                )
        ...
        if self.parameters.period != 0:
            picked = numpy.array(
                [
                    i
                    for i in range(0, len(z))
                    if abs(vals[i] - self.parameters.value) < self.parameters.distance
                ]
            )
        else:
            picked = numpy.array(
                [
                    ...
                    for i in range(0, len(z))
                    if circular_dist(
                        vals[i], self.parameters.value, self.parameters.period
                    )
...
class SimilarAnnotationSelectorRegion(SimilarAnnotationSelector):
    ...
    *annotation* value is closer than *distance* from pre-specified *value*
    ...
    def generate_idd_list_of_neurons(self):
        ...
        xx, yy = self.sheet.cs_2_vf(
            self.sheet.pop.positions[0], self.sheet.pop.positions[1]
        )
        picked_region = set(
            numpy.arange(0, len(xx))[
                numpy.logical_and(
                    (
                        abs(
                            numpy.array(xx - self.parameters.offset_x)
                            < self.parameters.size / 2.0
                        ),
                        abs(
