        ...
        if self.parameters.exc_firing_rate != 0 and self.parameters.exc_weight != 0:
            for j, i in enumerate(self.to_stimulate_indexes):
                if self.parameters.drive_period < duration:
                    z = numpy.arange(
                        self.parameters.drive_period + 0.001, duration - 100, 10
                    )
                    times = [0] + z.tolist()
                    rate = [self.parameters.exc_firing_rate] + (
                        (1.0 - numpy.linspace(0, 1.0, len(z)))
                        * self.parameters.exc_firing_rate
                    ).tolist()
                else:
                    times = [0]
                    rate = [self.parameters.exc_firing_rate]
                pp = (
                    ...
                    .inh_poisson_generator(
                        numpy.array(rate), numpy.array(times), t_stop=duration
                    )
                    .spike_times
                )
                a = offset + numpy.array(pp)
...
class Depolarization(DirectStimulator):
    ...
    def prepare_stimulation(self, duration, offset):
        self.scs.set_parameters(
            times=[offset + self.sheet.dt * 2], amplitudes=[self.parameters.current]
        )
...
class LocalStimulatorArray(DirectStimulator):
    ...
    def __init__(self, sheet, parameters, shared_scs=None):
        ...
        assert (
            math.fmod(self.parameters.size, self.parameters.spacing) < 0.000000001
        ), "Error the size has to be multiple of spacing!"
        assert (
        ...
        xx, yy = self.sheet.vf_2_cs(
            self.sheet.pop.positions[0], self.sheet.pop.positions[1]
        )
        ...
        for depth in numpy.arange(
            ...
            assert numpy.shape(
                temp[n - cutof : n + cutof + 1, n - cutof : n + cutof + 1]
            ) == (2 * cutof + 1, 2 * cutof + 1), (
                str(numpy.shape(temp[n - cutof : n + cutof, n - cutof : n + cutof]))
                + "vs"
                + str((2 * cutof + 1, 2 * cutof + 1))
            )
            mixing_templates.append(
                (temp[n - cutof : n + cutof + 1, n - cutof : n + cutof + 1], cutof)
            )

        signal_function = load_component(self.parameters.stimulating_signal)
        stimulator_signals, self.scale = signal_function(
            sheet,
        ...
        nearest_iz = numpy.rint(
            (numpy.array(self.sheet.pop.positions[2]) - sheet.parameters.min_depth)
            / self.parameters.depth_sampling_step
        )
        ...
        for i in range(0, self.sheet.pop.size):
            ...
            if ss != numpy.array([]):
                temp = temp[
                    max(int(cutof - nearest_ix[i]), 0) : max(
                        int(2 * n + 1 + cutof - nearest_ix[i]), 0
                    ),
                ...
                self.mixed_signals[i, :] = (
                    ...
                    * W
                    * numpy.dot(
                        temp.flatten(), numpy.reshape(ss, (len(temp.flatten()), -1))
                    )
                )

        lam = numpy.squeeze(numpy.max(self.mixed_signals, axis=1))
        for i in range(0, self.sheet.pop.size):
            self.sheet.add_neuron_annotation(
                i,
                "Light activation magnitude("
                + self.sheet.name
                + ","
                + str(self.scale)
                + ","
                + str(self.parameters.stimulating_signal_parameters.orientation.value)
                + ","
                + str(self.parameters.stimulating_signal_parameters.sharpness)
                + ","
        ...
        assert numpy.shape(self.mixed_signals) == (
            ...
            + " vs "
            + str((self.sheet.pop.size, numpy.shape(stimulator_signals)[1]))
        )

        self.stimulation_duration = (
            numpy.shape(self.mixed_signals)[1] * self.parameters.current_update_interval
        )
        ...
        if shared_scs != None:
            ...
            self.scs = [
                self.sheet.sim.StepCurrentSource(times=[0.0], amplitudes=[0.0])
                for cell in self.sheet.pop.all_cells
            ]
    ...
    def prepare_stimulation(self, duration, offset):
        assert self.stimulation_duration == duration, (
            "stimulation_duration != duration :"
            + str(self.stimulation_duration)
            + " "
            + str(duration)
        )
        times = (
    ...
    def inactivate(self, offset):
        for scs in self.scs:
            scs.set_parameters(
                times=[offset + 3 * self.sheet.dt], amplitudes=[0.0], copy=False
            )
...
class LocalStimulatorArrayChR(LocalStimulatorArray):
    ...
    def __init__(self, sheet, parameters, shared_scs=None):
        ...
        for i in range(0, self.sheet.pop.size):
            self.sheet.add_neuron_annotation(
                i,
                "Light activation magnitude ChR("
                + str(self.scale)
                + ","
                + str(self.parameters.stimulating_signal_parameters.orientation.value)
                + "_"
...
def test_stimulating_function(
    sheet, coor_x, coor_y, current_update_interval, parameters
):
...
def test_stimulating_function_Naka(
    sheet, coor_x, coor_y, current_update_interval, parameters
):
    z = sheet.pop.all_cells.astype(int)
    vals = numpy.array(
