...
def perform_analysis_and_visualization(data_store):
    analog_ids = sorted(param_filter_query(data_store,sheet_name="Exc_Layer").get_segments()[0].get_stored_vm_ids())
    analog_ids_inh = sorted(param_filter_query(data_store,sheet_name="Inh_Layer").get_segments()[0].get_stored_vm_ids())
    ...
    if True: # PLOTTING
            ...
            PSTH(param_filter_query(data_store,st_direct_stimulation_name="Kick"),ParameterSet({'bin_length' : 5.0})).analyse()
            TrialAveragedFiringRate(param_filter_query(data_store,st_direct_stimulation_name="Kick"),ParameterSet({})).analyse()
            Irregularity(param_filter_query(data_store,st_direct_stimulation_name="Kick"),ParameterSet({})).analyse()
            ...
            dsv = param_filter_query(data_store,st_direct_stimulation_name=['Kick'])    
