...
from MozaikLite.visualization.plotting import Plotting,RasterPlot,GSynPlot,VmPlot,ConductanceSignalListPlot 
...
class Figure2(Plotting):
      ...
      def subplot(self,subplotspec):
          ...
          RasterPlot(dsv,ParameterSet({'sheet_name' : self.parameters.sheet_name, 'trial_averaged_histogram' : True})).subplot(gs[0:4,:5])
          ...
          ConductanceSignalListPlot(queries.TagBasedQuery(ParameterSet({'tags' : ['GSTA1'] })).query(self.datastore),ParameterSet({'sheet_name' : 'V1_Exc'})).subplot(gs[2:6,5:])  
